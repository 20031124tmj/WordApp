...
const Review: React.FC = () => {
  ...
  if (words.length === 0) return (
    <div style={{ textAlign: 'center', marginTop: 80 }}>
      <p style={{ fontSize: 18, color: '#666' }}>没有待复习的单词</p>
      <p style={{ fontSize: 14, color: '#999', marginTop: 8 }}>学过的单词到期后会自动出现在这里</p>
      <button onClick={() => navigate('/')} style={{ marginTop: 16, padding: '8px 24px', borderRadius: 8, border: 'none', backgroundColor: '#6c63ff', color: '#fff', cursor: 'pointer', fontSize: 16 }}>返回首页</button>
    </div>
  );
